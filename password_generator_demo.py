...
#! datetime - позволяет узнавать время и дату
#! string - библиотека символов
...
def get_password_len():
    while True:
        try:
            password_len = int(input("Введите длинну пароля (от 6 до 16):"))
            if (password_len >= 6) and (password_len <= 16):
                return password_len
            else:
                print("Введите числа в интеврале (от 6 до 16)")
        except ValueError:
...
def save_password(password):
    ...
    if want_to_save.lower() == "да":
        ...
        print("Хорошо, удачи! Удачи вспоминать его, когда понадобится :)")
        return
    
    print('Успешно все сохранили!!!')
    return
